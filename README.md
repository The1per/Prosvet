...
Отзыв тем же заходом пересылается ботом в телеграм — если заданы две
переменные окружения (Settings → Environment variables, как Secret):

    TG_TOKEN=1234567890:AAH...     токен от @BotFather
    TG_CHAT=123456789              номер разговора

Без них пересылки просто нет, а запись в базу идёт как обычно. Отправка
уходит в waitUntil и не может ни задержать ответ странице, ни сломать её.
