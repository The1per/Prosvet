      ...
      <main className="relative z-10 mx-auto max-w-[1920px] px-2 pb-8 pt-2 sm:px-3">
        ...
        >
          ...
          >
            ...
            >
              ...
              <div className={телефон ? "w-full" : "shrink-0"} style={телефон ? undefined : { width: ШИРИНА_ЧИСЛА }}>
                ...
                <div className="mono mt-1.5">
                  ...
                  {/* ПАРА ФОМа СТОИТ В СТОЛБЦЕ ЧИСЛА, сразу под строкой места.
                      Своей строкой во всю карточку она отбирала у страницы
                      целый ряд ради двух коротких вещей, а справа от неё
                      оставалась пустота во всю ширину. */}
            ...
            {/* СЛОВА НЕДЕЛИ ВСТАЮТ В ТУ САМУЮ ПУСТОТУ, что оставлял ступенчатый
                верхний ряд: строй кончается на 138, сравнения на 168, а столбец
                числа тянется до 204, и справа под ними полоса в 780 пикселей
                пустовала. Отступ слева ровняет их по строю.

                Поэтому же строки идут ДРУГ ПОД ДРУГОМ, а не в ряд: в ряд самая
                широкая неделя просит 1150 пикселей, а здесь их 780. Две строки
                просят 620 -- влезают с запасом, и высота блока от недели не
                зависит: строк всегда две.

                ПОЛОЖЕНИЕ ЗАКРЕПЛЕНО. Подписи стоят колонкой постоянной ширины,
                и слова обеих строк начинаются на одной вертикали в любую
                неделю. Ни ширина блока, ни его высота от недели не меняются --
                при быстром перелистывании ничего не ездит. */}
            {!телефон && (
              <div className="-mt-[70px] pl-[376px]">
...
function СловаНедели({ w, lang, phone = false }: { w: Week; lang: Lang; phone?: boolean }) {
  ...
  return (
    <div className="relative" ref={коробка}>
      ...
      <div
        className={
          "mt-0.5 flex flex-col " + (phone ? "gap-0.5" : "gap-1")
        }
      >
        {ряды.map((р) => (
