...
export default function App() {
  ...
  /**
   * ВВОДНАЯ ПОДСКАЗКА «КАК ЭТО РАБОТАЕТ».
   *
   * Человек открывает страницу и видит число. Откуда оно -- не сказано нигде,
   * кроме подписи мелким шрифтом, и раздел с разбором лежит ниже трёх экранов.
   * Поэтому при ПЕРВОМ заходе рядом с числом всплывает чёрная плашка, и
   * нажатие на неё уводит прямо в разбор.
   *
   * ОДИН РАЗ, А НЕ КАЖДЫЙ. Отметка в браузере: показали -- больше не зовём.
   * Дальше её всегда можно вызвать наведением на само число (на компьютере).
   * Гаснет от любого нажатия, как и прочие подсказки на странице.
   */
  const [какПодсказка, setКакПодсказка] = useState(false);
  useEffect(() => {
    try {
      if (localStorage.getItem("pai.как.v1")) return;
      localStorage.setItem("pai.как.v1", "1");
    } catch {
      /* хранилище закрыто -- покажем, это не беда */
    }
    setКакПодсказка(true);
  }, []);
  /* СТОРОЖ ПРОПУСКАЕТ НАЖАТИЕ ПО САМОЙ ПОДСКАЗКЕ. Без этой оговорки он гасил
     её на pointerdown -- раньше, чем до неё доходило нажатие, -- и подсказка
     просто исчезала, никуда не уводя. Ровно на это и пожаловался хозяин. */
  useEffect(() => {
    if (!какПодсказка) return;
    let снять = () => {};
    const т = setTimeout(() => {
      const закрыть = (e: Event) => {
        const ц = e.target as HTMLElement | null;
        if (ц && ц.closest && ц.closest("[data-подсказка]")) return;
        setКакПодсказка(false);
      };
  ...
  const парФОМ = (
    ...
    >
      ...
      {/* ВВОДНАЯ ПОДСКАЗКА НА КОМПЬЮТЕРЕ -- ПРЯМО НА ПАРЕ ОПРОСА (решение
          хозяина: «ниже и левее, прям на опрос ФОМ»). Ставится внутри самой
          пары, поэтому попадает на неё при любом масштабе страницы, а не по
          угаданным пикселям. */}
      {!телефон && какПодсказка && (
        <ПодсказкаКак
          lang={lang}
          phone={false}
          жать={кМетоду}
                ...
                <div className="flex flex-col">
                  ...
                  <div className="flex items-end gap-3">
                    ...
                    <span
                      ...
                      /* На компьютере наведение на число снова показывает
                         подсказку «как это работает»: вводную гасят одним
                         нажатием, а вопрос «откуда это число» остаётся. */
                      onMouseEnter={телефон ? undefined : () => setКакПодсказка(true)}
                      onClick={кМетоду}
                    >
                      {shown.toFixed(1)}
                    </span>
                    ...
                    <span className={"relative whitespace-nowrap " + (телефон ? "pt-2 pl-4 text-[15px]" : "pb-1 text-[17px]")} style={{ color: "var(--ink-2)" }}>
                      {/* НА ТЕЛЕФОНЕ -- СПРАВА ОТ ЭТОЙ ПОДПИСИ, в две строки
                          (решение хозяина). В одну строку плашке нужно 122
                          пикселя, а от конца подписи до края экрана остаётся
                          79: поэтому кегль меньше и ширина закреплена, слова
                          переносятся сами. Абсолютная -- строку не двигает. */}
                      {телефон && какПодсказка && (
                        <ПодсказкаКак
                          lang={lang}
                          phone
...
/**
 * Плашка «Как это работает»: зовёт в разбор и сама туда ведёт.
 *
 * Чёрная, как полоса шапки, с пунктиром под словами -- чтобы читалась кнопкой,
 * а не надписью. Ставится абсолютно: где бы ни появилась, раскладку она не
 * двигает и ничего под собой не ломает.
 */
function ПодсказкаКак({
  lang, phone, className = "", жать,
}: { lang: Lang; phone: boolean; className?: string; жать: () => void }) {
  return (
    <button
      type="button"
      /* Метка для сторожа: нажатие ПО ПОДСКАЗКЕ он пропускает, иначе она
         гаснет раньше, чем успевает сработать. */
      data-подсказка="как"
      onClick={жать}
      className={
        "z-40 rounded-xl border " + className + " "
        + (phone
            /* На телефоне в две строки: справа от подписи ширины на одну не
               хватает, а справа хозяин и просил. */
            ? "w-[86px] whitespace-normal px-2 py-1 text-[12px] leading-tight"
            : "whitespace-nowrap px-3.5 py-2 text-[16px]")
      }
      style={{
        borderColor: "var(--line-strong)",
        background: "var(--bg-2)",
...
function НастройкиВида({
  ...
  const ряды: { к: "простой" | "индекс" | "фом" | "прежнее" | "превышение"
    ...
    { к: "индекс", знак: "∿", имя: ru ? "кривая модели" : "index curve", вкл: значения.индекс },
