...
export default function App() {
  ...
  useEffect(() => {
    ...
    let снять = () => {};
    const т = setTimeout(() => {
      const закрыть = () => setКакПодсказка(false);
          ...
          >
            ...
            >
              ...
              <div className={"relative " + (телефон ? "w-full" : "shrink-0")} style={телефон ? undefined : { width: ШИРИНА_ЧИСЛА }}>
                {/* НА КОМПЬЮТЕРЕ -- ПОД ЧИСЛОМ, на месте пары опроса: там в
                    этот миг ничего срочного, а взгляд уже стоит на числе. */}
                ...
                <div className="flex flex-col">
                  ...
                  <div className="flex items-end gap-3">
                    ...
                    <span className={"relative whitespace-nowrap " + (телефон ? "pt-2 pl-4 text-[15px]" : "pb-1 text-[17px]")} style={{ color: "var(--ink-2)" }}>
                      {/* НА ТЕЛЕФОНЕ ПОДСКАЗКА У ЭТОЙ ЖЕ ПОДПИСИ, но не сбоку
                          от неё, а сразу под ней и по правому краю. Сбоку она
                          не помещается: плашке нужно 122 пикселя, а от конца
                          подписи до края экрана остаётся 79 -- проверено
                          замером, она уходила за край на 35.
                          Абсолютная: строку не двигает, карточку не
                          удлиняет. */}
                      {телефон && какПодсказка && (
                        <ПодсказкаКак
                          lang={lang}
...
function ПодсказкаКак({
  ...
  return (
    <button
      ...
      onClick={жать}
      className={
        "z-40 whitespace-nowrap rounded-xl border " + className + " "
        + (phone ? "px-2.5 py-1.5 text-[13px]" : "px-3.5 py-2 text-[16px]")
      }
      style={{
        borderColor: "var(--line-strong)",
        background: "var(--bg-2)",
