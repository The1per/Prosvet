...
// ВЕРХНЕЕ ПОЛЕ: 96 -> 74 -> 58. Столько держат подписи событий вместе с
// полочками, которые тянутся от них к кривой. Каждое сокращение отдаёт разницу
// самой кривой: 58 -- это ряд подписей и короткая полочка, ниже уже начнут
// налезать друг на друга подпись и точка, к которой она ведёт.
const PAD_БАЗА = { l: 34, r: 12, t: 58, b: 30 };
...
// ЗАПАСНЫЕ ГРАНИЦЫ ОСИ. Действующие считаются по тому, что на графике
// показано (см. ось в самом Chart). Эти остаются на случай, когда все кривые
// выключены и мерить нечего: без них поле схлопнулось бы в ноль высоты.
const LO_ЗАПАС = 26;
const HI_ЗАПАС = 76;
const TICKS_ЗАПАС = [30, 40, 50, 60, 70];
// Запас над крайними значениями, в пунктах опроса: кривая не должна липнуть к
// верхнему и нижнему краю поля. Три, а не шесть: с шестью полный вид выходил
// 10…80 против прежних 26…76, то есть кривая становилась ПЛОЩЕ, чем была, --
// ровно наоборот тому, зачем живая ось заводилась.
const ЗАПАС_ОСИ = 3;
// Границы округляются наружу по двойке -- мелко, чтобы поле обнимало значения.
// Шаг ДЕЛЕНИЙ выбирается отдельно и крупно: если округлять границы по шагу
// делений, полный ряд прыгает до 10…80 и половина поля стоит пустой.
const КРАТНОСТЬ_ГРАНИЦ = 2;
...
type Props = {
  ...
  индекс?: boolean;
  /**
   * КАК БЫЛО В ПРОШЛУЮ ПУБЛИКАЦИЮ. Нажал -- и вся кривая стала прежней:
   * не вторая линия рядом, а замена. Числа берутся из самого файла прошлой
   * публикации (поле idx0), а не пересчитываются, поэтому на графике
   * оказывается ровно то, что там стояло.
   */
  прежнее?: boolean;
  /**
...
export default function Chart({ data: сырые, lang, sel, onSel, showFom, простой = false, индекс = true, прежнее = false, превышение = false, сдержанно = false, phone = false, уголок }: Props) {
  /**
   * ПОДМЕНА НА ПРЕЖНЕЕ ДЕЛАЕТСЯ ОДИН РАЗ, У ВХОДА. Дальше по всему полотну --
   * кривая, свечение, кружки, табличка недели -- идёт уже подменённый ряд, и
   * ни одно место ниже об этом знать не обязано. Неделя, для которой прошлого
   * значения нет, остаётся как есть: выдумывать его нельзя.
   */
  const data = useMemo(
    () =>
      прежнее
        ? сырые.map((d) => {
            const п = (d as { idx0?: number | null }).idx0;
            return п == null ? d : { ...d, idx: п };
          })
        : сырые,
    [сырые, прежнее],
  );
  ...
  /**
   * ОСЬ ПОДСТРАИВАЕТСЯ ПОД ПОКАЗАННОЕ, А РАЗМЕР ПОЛЯ НЕ МЕНЯЕТСЯ.
   *
   * Было наглухо 26…76 на все годы. Диапазон режет ряд -- и внутри спокойного
   * года кривая шла тонкой полоской в середине поля: половина высоты уходила
   * на значения, которых в показанном куске нет вовсе. Теперь границы берутся
   * у самих значений, а высота и ширина полотна остаются те же, что были:
   * меняется цена пикселя, не размер картинки.
   *
   * СЧИТАЕТСЯ ПО ТЕМ КРИВЫМ, КОТОРЫЕ ВИДНЫ. Включил превышение или сдержанный
   * вид -- их крайние значения тоже входят, иначе включённая линия уехала бы
   * за край поля. Выключил всё -- работают запасные границы.
   *
   * ГРАНИЦЫ КРУГЛЫЕ. Сырые min и max дали бы деления вида «37.4», и подписи
   * оси читались бы как показания. Шаг выбирается по размаху -- 10, 5 или 2, --
   * и границы округляются наружу по этому шагу.
   *
   * ЧЕГО ЭТО НЕ ТРОГАЕТ: ни одного числа. Показание, место недели и все
   * сравнения считаются вне графика; здесь меняется только то, на какой
   * высоте нарисована точка.
   */
  const ось = useMemo(() => {
    const vs: number[] = [];
    for (const d of data) {
      if (индекс && Number.isFinite(d.idx)) vs.push(d.idx);
      if (превышение) {
      ...
      /**
       * ОПРОС ВХОДИТ В ГРАНИЦЫ ВСЕГДА, ПОКАЗАН ОН ИЛИ НЕТ.
       *
       * Раньше он входил только когда включён -- и нажатие на «кривую опроса»
       * меняло верх шкалы, отчего вся картинка разом сжималась по высоте.
       * Кривая индекса при этом не менялась ни на число, а выглядела иначе, и
       * читалось это как «прибор поехал». Теперь поле сразу вмещает опрос, и
       * переключатель ничего не двигает: включается и выключается только
       * линия.
       *
       * Цена честная и небольшая: при выключенном опросе поле чуть выше, чем
       * нужно самой кривой. Это лучше, чем прыжок.
       */
      if (d.fom != null) vs.push(d.fom);
    }
    if (!vs.length) {
      return { LO: LO_ЗАПАС, HI: HI_ЗАПАС, TICKS: TICKS_ЗАПАС };
    }
    const к = КРАТНОСТЬ_ГРАНИЦ;
    let lo = Math.floor((Math.min(...vs) - ЗАПАС_ОСИ) / к) * к;
    let hi = Math.ceil((Math.max(...vs) + ЗАПАС_ОСИ) / к) * к;
    // Вырожденный случай: одна неделя в диапазоне, размах ноль. Поле обязано
    // иметь высоту, иначе делить на нуль.
    if (hi - lo < 4 * к) hi = lo + 4 * к;
    const размах = hi - lo;
    // Порог 40, а не 60: при 60 полный ряд (размах 56) шёл шагом 5 и давал
    // одиннадцать пунктирных линий вместо прежних пяти -- поле в клетку.
    const шаг = размах > 40 ? 10 : размах > 18 ? 5 : 2;
    const ticks: number[] = [];
    for (let v = Math.ceil((lo + 1) / шаг) * шаг; v < hi - 1e-9; v += шаг) {
      ticks.push(Math.round(v));
    }
    return { LO: lo, HI: hi, TICKS: ticks };
  }, [data, индекс, превышение, сдержанно]);
  const { LO, HI, TICKS } = ось;
  ...
  // искал её заново.
  //
  // НА ЭКРАНЕ ЕЁ ВЕРХНИЙ КРАЙ ПРИВЯЗАН К ОСИ -- НА ДВА ПУНКТА ВЫШЕ ВЕРХНЕЙ
  // ГРАНИЦЫ. Прежде она стояла на PAD.t - 52, то есть в самом верху полотна,
  // ВНУТРИ ряда подписей событий, и накрывала их. Было жёстко y(78) при
  // границе 76; теперь граница живая, и привязка к ней сохраняет то же
  // положение при любом диапазоне: чуть выше поля, ниже подписей.
  //
  // На телефоне всё как было: там верхнее поле 26 пикселей, подписей событий
  // в поле нет вовсе (они ушли в ПолосуСобытий над графиком), и опускать
  // табличку некуда и незачем.
  const boxY = phone ? PAD.t - 52 : y(HI + 2);
  const boxH = 44;
    ...
    <div className="relative flex w-full flex-1 select-none" draggable={false} onDragStart={(e) => e.preventDefault()}>
      ...
      >
        <defs>
          ...
          {/* ТОТ ЖЕ СВЕТ, НО ОБЛАСТЬЮ ВО ВСЁ ПОЛЕ, А НЕ В ДОЛЯХ ОТ КУСКА.
              12 сентября 2026: у длинных событий вокруг свечения была видна
              строго прямоугольная кромка. Дело не в самом свете, а в области
              фильтра: заданная в процентах, она мерится от рамки куска, а у
              длинного события рамка низкая и плоская -- шесть-восемь пикселей
              высотой. Шестьдесят процентов от такой рамки это четыре пикселя,
              а размытию нужно пятнадцать: остальное отрезалось по краю
              области, и отрез шёл ровной чертой сверху и снизу. Здесь область
              задана в координатах самого поля и одинакова для любого куска --
              обрезать нечего, и ореол сходит на нет сам, кругло.
              Для главной кривой прежний фильтр годится: её рамка -- почти всё
              поле, и проценты от неё дают запас с избытком. */}
          <filter id="glowW" filterUnits="userSpaceOnUse"
                  x={0} y={-40} width={W} height={H + 80}>
            <feGaussianBlur stdDeviation="5" result="b" />
            <feMerge>
              <feMergeNode in="b" />
              <feMergeNode in="SourceGraphic" />
        ...
        {индекс && полосы.map((п) => (
          <path
            ...
            /* Всегда со свечением: цвет события -- это показание, а не
               ...
               Фильтр -- с областью во всё поле (#glowW): у долгих событий
               процентная область резала ореол прямоугольником. */
            filter="url(#glowW)"
          />
        ))}
