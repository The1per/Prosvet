...
export default function Chart({ data, lang, sel, onSel, showFom, простой = false, индекс = true, превышение = false, сдержанно = false, phone = false, уголок }: Props) {
  ...
  // искал её заново.
  //
  // НА ЭКРАНЕ ЕЁ ВЕРХНИЙ КРАЙ ПРИВЯЗАН К ОСИ Y, К УРОВНЮ 78. Прежде она
  // стояла на PAD.t - 52, то есть в самом верху полотна, ВНУТРИ ряда подписей
  // событий, и накрывала их. Привязка к делению оси, а не к отступу полотна,
  // держит её на одном месте при любой высоте верхнего поля: 78 лежит выше
  // всей кривой (её потолок 68.7) и ниже подписей, поэтому табличка не
  // закрывает ни то, ни другое.
  //
  ...
  const boxY = phone ? PAD.t - 52 : y(78);
  const boxH = 44;
