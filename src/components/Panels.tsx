 ...
 * ФАМИЛИЯ И ИНИЦИАЛЫ.
 *
 * В источнике люди названы по-каталожному: «Партон, Долли», «Цурило, Юрий
 * Алексеевич». Полное имя в списке занимает всю строку и вытесняет число
 * просмотров, ради которого список и стоит. Поэтому «Партон Д.» и
 * «Цурило Ю. А.» -- узнаётся так же, а места втрое меньше.
 *
 * Сокращаем ОСТОРОЖНО: только если запятая ровно одна, а то, что после неё,
 * похоже на имя -- начинается с большой буквы и не содержит ни цифр, ни
 * скобок. «Список умерших в 2026 году» и «Я — легенда (фильм)» не тронутся.
 */
...
function Жирным({ t }: { t: string }) {
  // Пустая строка разбивает текст на абзацы: сплошной кусок в десять строк
  // читается как стена, а мысли в нём разные.
  const абзацы = t.split(String.fromCharCode(10, 10));
  if (абзацы.length > 1)
    return (
      <>
        {абзацы.map((а, i) => (
          <span key={i} className={i ? "mt-3 block" : "block"}>
            <Жирным t={а} />
          </span>
        ))}
      </>
