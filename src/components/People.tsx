...
export default function People({ idx, lang, preview = null, part }: Props) {
  ...
  const floorPx = whole * (W + GAP) + (floor - whole) * W;
  ...
  if (part === "подпись") {
    return (
      <div className="space-y-1" style={{ maxWidth: PEOPLE_WIDTH + 110 }}>
        ...
        <div className="mono text-[17px] leading-snug" style={{ color: "var(--ink-3)" }}>
          ↳ {floorLabel}
        </div>
      </div>
    );
  ...
  return (
    <div className="relative inline-block" style={{ width: PEOPLE_WIDTH }}>
      <div className="flex items-end" style={{ gap: GAP }} role="img" aria-label={label}>
