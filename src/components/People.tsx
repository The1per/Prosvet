 ...
 *
 * ОТСЕЧКИ МИН И МАКС -- ЖИВЫЕ, ОТ САМОГО ГРАФИКА. Прежде они стояли на
 * измеренных пределах ОПРОСА за все годы (3,2 и 7,0 из десяти) и не
 * двигались никогда. Теперь они приходят снаружи, из того куска ряда,
 * который показан на графике: сдвинул диапазон -- отсечки переехали, и строй
 * говорит о той же истории, что и кривая рядом с ним. Считаются той же
 * `anxiousOfTen`, что заливает фигуры, поэтому стоят с ними на одной шкале.
 *
 * Пределы опроса (FLOOR_FOM, PEAK_FOM) остались запасом: если снаружи
 * ничего не пришло, отсечки встают на них, как было.
 *
...
type Props = {
  ...
  lang: Lang;
  /**
   * Границы отсечек в «из десяти». Приходят из App -- по тому куску ряда,
   * который сейчас на графике. Не заданы -- берутся пределы опроса.
   */
  floor?: number | null;
  peak?: number | null;
...
export default function People({ idx, lang, preview = null, part, phone = false, level,
  floor: полСнаружи = null, peak: потолокСнаружи = null }: Props) {
  ...
  // ПОЛ И ПОТОЛОК -- ИЗ ПОКАЗАННОГО КУСКА РЯДА, а не назначенные. Запас --
  // измеренные пределы опроса: за одиннадцать лет он не опускался ниже 32 %
  // и не поднимался выше 70 %.
  //
  // Порядок на случай, если снаружи придёт перевёрнутая пара: min/max берутся
  // у самих значений, а не у их имён. Одна строка, зато отсечка «мин.» не
  // сможет оказаться правее «макс.».
  const пол0 = полСнаружи ?? FLOOR_FOM / 10;
  const пот0 = потолокСнаружи ?? PEAK_FOM / 10;
  const floor = Math.min(пол0, пот0);
  const peak = Math.max(пол0, пот0);
  ...
  return (
    ...
    <div className="relative inline-block" style={{ width: ширина }}>
      ...
      {/* ОДИН ПРОМЕЖУТОК НА ДВЕ ОТСЕЧКИ -- ОДНА ОТСЕЧКА. Годами диапазон
          уже одного года не задаётся, и там пределы всегда расходятся (мерено:
          ни у одного из восьми годов отсечки не совпадают). Но календарём
          можно взять и две недели, и тогда min и max попадают в один и тот же
          промежуток между фигурами -- две подписи встали бы одна на другую.
          Тогда рисуется одна, и сказано в ней ровно то, что есть: весь
          показанный кусок укладывается между этими двумя людьми. */}
      {(вПромежуток(floor) === вПромежуток(peak)
        ? [[ru ? "мин./макс." : "min/max", вПромежуток(floor)] as const]
        : [["мин.", вПромежуток(floor)] as const,
           ["макс.", вПромежуток(peak)] as const]
      ).map(([метка, x]) => (
        <div
          key={метка as string}
